require(['JBrowse/Store/RemoteBinaryFile','dojo/aspect']
, function( RemoteBinaryFile, aspect ) {
    ...
    describe( 'RemoteBinaryFile', function() {
        ...
        var testURL = '../data/raw/volvox/volvox-sorted.bam';

        it('constructs', function () { expect(b).toBeTruthy(); });

        it('appears to fetch data correctly', function (done) {
            var get = function( start, end, callback ) {
                b.get({ url: testURL, start: start, end: end, success: function (d) { callback(d); }, failure: console.error });
            };
